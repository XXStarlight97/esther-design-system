...
| 色名 | 色值 | 对应 | 用途 |
|------|------|------|------|
| 深松绿 | `#306848` | 头发 | 主色调、标题、超链接、重点标记 |
| 粉紫 | `#D090B8` | 裙子 | 强调、装饰、badges、连接线、高亮 |
| 柠檬黄 | `#F4D758` | 蝴蝶结/鞋 | 点缀、高亮下划线、CTA、标签 |

三色比例原则：深松绿60% · 粉紫30% · 柠檬黄10%（柠檬黄永远是点缀，不做主色）

---

## 👧 IP形象规范

星星电灯Starlight的固定视觉形象（face.png），以下元素不可更改：

- **配色** — 品牌识别第一要素
- **名称** — 不可修改

### 素材文件
- **头像** — `assets/face.png`（深绿色底+白色starlight，适用于个人介绍区、头像集群等）
- **全身形象** — `image_face.png`（完整IP形象）

### 使用规则
- 不允许修改形象比例、颜色、元素
- 是否在页面中使用IP形象由用户决定，不强制
- 需要头像时优先用 `assets/face.png`
...
| 类型 | 禁止 |
|------|------|
| 配色 | 蓝紫渐变、cyan、neon、纯黑白、AI常用的冷灰蓝调、黑色/深色版面（仅3:4卡片场景禁止，HTML全屏页面不受限）、大面积粉紫与柠檬黄不能同时作为主色 |
...
- **选中文本高亮**: `::selection { background: #D090B8; color: #1a1a1a; }`
- **对话气泡配色**（如有对话场景）：Agent=深松绿底白字 `#306848`，Starlight=粉紫底黑字 `#D090B8`
- **边框装饰**: 粉紫装饰边框统一40px
- **链接悬停**: 用粉紫底色块或下划线，不用变色
